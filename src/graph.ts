    ...
    for (var obj of this.tileIndex.objectIndex.values()) {
      ...
      if (geomInstance(obj)) {
        ...
        osmRootElem.push({
          ...
          }, { tag: { _attr: { k: 'oneway', v: oneWay } } }, ...nodeIdElems]
        });
  ...
  async buildGraph() {
    ...
    try {
      ...
      if (USE_LOCAL_CACHE && existsSync(dbPath)) {
        ...
        console.log(chalk.green("     loading pre-built " + this.graphMode + " graph from: " + osrmPath));
        ...
        console.log(chalk.green("     building graph using OSRM from: " + OSRM_DIR));


        mkdirSync(dbPath, { recursive: true });
        this.db = new LevelDB(dbPath)

        console.log(chalk.green("     building " + this.graphMode + " graph  xml..."));
        var xmlPath = await this.createGraphXml();

        //extract
        console.log(chalk.green("     building " + this.graphMode + " graph from: " + xmlPath));
        ...
        var osrmPath: string = xmlPath + '.osrm';

        if (OPTIMIZE_GRAPH) {
          console.log(chalk.green("     optimizing graph..."));
          ...
          console.log(chalk.green("     skipping graph optimization..."));
          this.osrm = new OSRM({ path: osrmPath, algorithm: "MLD" });
        }
      }
    } catch (e) {
