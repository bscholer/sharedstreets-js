...
import { CleanedLines, CleanedPoints, reverseLineString } from '../geom.js';

import envelope from '@turf/envelope';
import { PathSegment, PointCandidate } from '../graph.js';

import { SharedStreetsIntersection, SharedStreetsReference } from 'sharedstreets-types';
import lineOffset from '@turf/line-offset';
import { generateBinId, getBinCountFromLength, getBinLength, getBinPositionFromLocation } from '../data.js';
...
export default class Match extends Command {
  ...
  static flags = {
    ...
    out: flags.string({
      char: 'o',
      description: 'file output name creates files [file-output-name].matched.geojson and [file-output-name].unmatched.geojson'
    }),
    ...
    'bearing-field': flags.string({
      description: 'name of optional point property containing bearing in decimal degrees',
      default: 'bearing'
    }),
    'search-radius': flags.integer({
      description: 'search radius for for snapping points, lines and traces (in meters)',
      default: 10
    }),
    'snap-intersections': flags.boolean({
      description: 'snap line end-points to nearest intersection if closer than distance defined by snap-intersections-radius ',
      default: false
    }),
    'snap-intersections-radius': flags.integer({
      description: 'snap radius for intersections (in meters) used when snap-intersections is set',
      default: 10
    }),
    ...
    'buffer-merge': flags.boolean({
      description: 'merge buffered points -- requires related buffer-merge-match-fields to be defined',
      default: false
    }),
    'buffer-merge-match-fields': flags.string({
      description: 'comma seperated list of fields to match values when merging buffered points',
      default: ''
    }),
    'buffer-merge-group-fields': flags.string({
      description: 'comma seperated list of fields to group values when merging buffered points',
      default: ''
    }),
    ...
    'join-points-match-fields': flags.string({
      description: 'comma seperated list of fields to match values when joining points',
      default: ''
    }),
    'join-point-sequence-field': flags.string({
      description: 'name of field containing point sequence (e.g. 1=start, 2=middle, 3=terminus)',
      default: 'point_sequence'
    }),

    'trim-intersections-radius': flags.integer({
      description: 'buffer and clip radius for intersections in point buffer and point join operations (in meters)',
      default: 0
    })
  }
  ...
  async run() {
    ...
    this.log(chalk.bold.green('  🌏  Loading geojson data...'));
    var inFile = args.file;

    var outFile = flags.out;

    if (!inFile || !existsSync(inFile)) {
      this.log(chalk.bold.yellow('  💾  Input file not found...'));
      return;
    }
    ...
    if (flags['direction-field'])
      console.log(chalk.bold.green('       Filtering one-way and two-way streets using field "' + flags['direction-field'] + '" with values: ' + ' "' + flags['one-way-with-direction-value'] + '", "' + flags['one-way-against-direction-value'] + '", "' + flags['two-way-value'] + '"'));

    if (flags['match-bike'] || flags['match-pedestrian']) {
      if (flags['match-bike']) {
        console.log(chalk.bold.green('       Matching using bike routing rules'));
      }
      if (flags['match-pedestrian']) {
        console.log(chalk.bold.green('       Matching using pedestrian routing rules'));
      }
      if (flags['match-motorway-only'])
        console.log(chalk.bold.yellow('       Ignoring motorway-only setting'));
    } else if (flags['match-car']) {
      if (flags['match-motorway-only'])
        console.log(chalk.bold.green('       Matching using car routing rules on motorways only'));
      else if (flags['match-surface-only'])
        console.log(chalk.bold.green('       Matching using car routing rules on surface streets only'));
      else
        console.log(chalk.bold.green('       Matching using car routing rules on all streets'));
    }
...
async function matchPoints(outFile, params, points, flags) {
  ...
  console.log(chalk.bold.green('  ✨  Matching ' + points.features.length + ' points...'));
  ...
  class MatchedPointType {
    originalFeature: turfHelpers.Feature<turfHelpers.Point>;
    matchedPoint: PointCandidate;
    bufferedPoint: PathSegment
  };
  ...
  if (flags['buffer-points']) {
    ...
    class MergeBufferedPointsType {
      mergedPathSegments: PathSegment;
      matchedPoints: MatchedPointType[]
    };

    console.log(chalk.bold.green('  ✨  Buffering ' + matchedPoints.length + ' matched points...'));

    var bufferLength = flags['buffer-points-length'];
    console.log(chalk.bold.green('        default buffer length: ' + bufferLength));
    ...
    if (flags['buffer-points-length-field']) {
      ...
      console.log(chalk.bold.green('        buffer length fieldname: ' + bufferLengthFieldName));
    }
    bufferLengthFieldName = flags['buffer-points-length-field'].toLocaleLowerCase().trim().replace(/ /g, "_");
    ...
    if (flags['buffer-merge']) {
      ...
      console.log(chalk.bold.green('  ✨  Merging ' + bufferedPoints.length + ' buffered points...'));
      ...
      if (flags['buffer-merge-match-fields']) {
        ...
        mergeFields = flags['buffer-merge-match-fields'].split(",").map((f) => {
          return f.toLocaleLowerCase().replace(/ /g, "_")
        });
        mergeFields.sort();
        console.log(chalk.bold.green('        merging on fields: ' + mergeFields.join(', ')));
      }

      let groupFields: string[] = [];
      if (flags['buffer-merge-group-fields']) {
        // split and clean property fields
        groupFields = flags['buffer-merge-group-fields'].split(",").map((f) => {
          return f.toLocaleLowerCase().replace(/ /g, "_")
        });
        groupFields.sort();
        console.log(chalk.bold.green('        grouping on field values: ' + groupFields.join(', ')));
      }
  ...
  if (flags['join-points']) {
    ...
    class JoinedPointsType {
      joinedPath: PathSegment;
      matchedPoints: MatchedPointType[]
    };

    console.log(chalk.bold.green('  ✨  Joining ' + matchedPoints.length + ' matched points...'));
    ...
    if (flags['join-points-match-fields']) {
      // split and clean property fields
      mergeFields = flags['join-points-match-fields'].split(",").map((f) => {
        return f.toLocaleLowerCase().replace(/ /g, "_")
      });
      mergeFields.sort();
      console.log(chalk.bold.green('        merging on fields: ' + mergeFields.join(', ')));
  ...
  if (matchedPoints.length) {
    console.log(chalk.bold.blue('  ✏️  Writing ' + matchedPoints.length + ' matched points: ' + outFile + ".matched.geojson"));
    var featureArray = []
    for (var matchedPoint of matchedPoints) {
  ...
  if (clusteredPoints.length) {
    console.log(chalk.bold.blue('  ✏️  Writing ' + clusteredPoints.length + ' clustered points: ' + outFile + ".clustered.geojson"));
  ...
  if (bufferedPoints.length) {
    console.log(chalk.bold.blue('  ✏️  Writing ' + bufferedPoints.length + ' buffered points: ' + outFile + ".buffered.geojson"));
  ...
  if (bufferedMergedPoints.length) {
    console.log(chalk.bold.blue('  ✏️  Writing ' + bufferedMergedPoints.length + ' buffered and merged points: ' + outFile + ".buffered.merged.geojson"));
  ...
  if (intersectionClusteredPoints.length) {
    console.log(chalk.bold.blue('  ✏️  Writing ' + intersectionClusteredPoints.length + ' intersection clustered points: ' + outFile + ".intersection_clustered.geojson"));
  ...
  if (unmatchedPoints.length) {
    console.log(chalk.bold.blue('  ✏️  Writing ' + unmatchedPoints.length + ' unmatched points: ' + outFile + ".unmatched.geojson"));
  ...
  if (joinedPoints.length) {
    console.log(chalk.bold.blue('  ✏️  Writing ' + joinedPoints.length + ' joined points: ' + outFile + ".joined.geojson"));
  ...
  if (cleanPoints.invalid && cleanPoints.invalid.length > 0) {
    console.log(chalk.bold.blue('  ✏️  Writing ' + cleanPoints.invalid.length + ' invalid points: ' + outFile + ".invalid.geojson"));
...
async function matchLines(outFile, params, lines, flags) {
  ...
  console.log(chalk.bold.green('  ✨  Matching ' + cleanedlines.clean.length + ' lines...'));
  ...
  if (matchedLines && matchedLines.length) {
    console.log(chalk.bold.blue('  ✏️  Writing ' + matchedLines.length + ' matched edges: ' + outFile + ".matched.geojson"));
  ...
  if (unmatchedLines && unmatchedLines.length) {
    console.log(chalk.bold.blue('  ✏️  Writing ' + unmatchedLines.length + ' unmatched lines: ' + outFile + ".unmatched.geojson"));
  ...
  if (cleanedlines.invalid && cleanedlines.invalid.length) {
    console.log(chalk.bold.blue('  ✏️  Writing ' + cleanedlines.invalid + ' in lines: ' + outFile + ".invalid.geojson"));
