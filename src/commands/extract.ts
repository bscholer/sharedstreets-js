...
import { TileIndex, TilePathParams, TileType } from '../index.js'
...
export default class Extract extends Command {
  ...
  async run() {
    ...
    if (flags.out)
      this.log(chalk.bold.green('  🌏  Loading polygon...'));
    ...
    this.log(chalk.bold.green('  🗄️  Loading SharedStreets tiles...'));
    ...
    this.log(chalk.bold.green('  🔍  Searching data...'));
    ...
    console.log(chalk.bold.blue('  ✏️  Writing ' + data.features.length + ' features to: ' + outFile + '.out.geojson'));
    ...
    if (flags['tiles']) {
      ...
      console.log(chalk.bold.blue('  ✏️  Writing ' + tiles.length + ' tile paths to: ' + outFile + '.tiles.txt'));
